...
use crate::{db, common::api_response::empty_ok, common::api_response::json_ok, utils, common::api_response::AppResult, common::api_response::EmptyResult, common::api_response::JsonResult};
...
pub async fn list_page(req: &mut Request, res: &mut Response) -> AppResult<()> {
    ...
    match is_fragment {
        Some(_) => {
            res.render(Text::Plain("User list fragment"));
        }
        None => {
            res.render(Text::Plain("User list page"));
        }
    }
    Ok(())
}
